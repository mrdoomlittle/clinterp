...
/*
            std::cout << "------|-> " << this-> current.bi_arguments[i].size() << std::endl;
            for (std::size_t o = 0; o != this-> current.bi_arguments[i].size(); o ++)
                std::cout << this-> current.bi_arguments[i][o];
            std::cout << "\n";       
*/
    ...
    char * get_bi_argument(std::size_t __arg_point) {
        ...
        memset(tmp, '\0', len * sizeof(char) + 1);
    ...
    char * get_bi_arg_value(char const * __bi_arg_name) {
        ...
        char * tmp = static_cast<char *>(malloc(len_of_val * sizeof(char) + 1));
        memset(tmp, '\0', len_of_val * sizeof(char) + 1);
