...
char **mdl::clinterp::filter(bool ignore) {
    ...
    for (size_t i = 0; i != term_input.size(); i ++) {
        if (this-> term_input[i] == STR_SEPARATOR || str_seporator) {
            if (!str_seporator) {
                seporator_mapping[i] = true;
                str_seporator = true;
            } else {
                if (this-> term_input[i] == STR_SEPARATOR) {
                    seporator_mapping[i] = true;
                    str_seporator = false;
                } else {
            ...
            if (this-> term_input[i] == ARG_SEPARATOR)
                seporator_mapping[i] = true;
            else
                seporator_mapping[i] = false;
            str_seporator = false;
    ...
    for (size_t i = 0; i != this-> term_input.size(); i ++) {
        if (begin_counting == true) {
            if (seporator_mapping[i])  {
                ...
                if (this-> term_input[i] == EQUALS_SIGN && ! tr) {skip = true; tr = true; }else skip = false;
    ...
    for (std::size_t o = 0 ; o != arg_len.size(); o ++) {
        ...
        char *__tmp = static_cast<char *>(malloc((arg_len[o][1] + 1) * sizeof(char)));
        memset(__tmp, '\0', (arg_len[o][1] + 1) * sizeof(char));
        for (std::size_t i = arg_len[o][0] ; i != (arg_len[o][0] + arg_len[o][1] + arg_len[o][2]); i ++) {
            //if (this-> term_input[i] == EQULS) eq ++;
            if (this-> term_input[i] == EQUALS_SIGN || found_val) {
                if (found_val) {
...
{
    bool res = false;
    char * tmp = static_cast<char *>(malloc((__value_0.size() + 1) * sizeof(char)));
    memset(tmp, '\0', (__value_0.size() + 1) * sizeof(char));
    for (size_t i = 0; i != __value_0.size(); i ++) tmp[i] = __value_0[i];
